...
if ($_SESSION['token']==$_POST['token']){
	...
	if($view == 2)	// coming from search_user.php(search with criteria)
	{
		if((!empty($search)) && ($search="yes"))
		{
			...
			$user_surname = escapeSimple($user_surname);
			$user_firstname = escapeSimple($user_firstname);
			$user_username = escapeSimple($user_username);
			$user_am = escapeSimple($user_am);
			$user_type = escapeSimple($user_type);
			$user_email = htmlspecialchars($user_email);
			$user_registered_at_flag = escapeSimple($user_registered_at_flag);
