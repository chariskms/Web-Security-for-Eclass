...
if ($_SESSION['token'] == $_POST['token']){
        $_POST['token'] = htmlspecialchars($_POST['token'], ENT_QUOTE);
					...
					while ($line !== false) {
							...
							if (!empty($line)) {
									...
									if (count($user) >= $numfields) {
											$info = array();
											foreach ($fields as $field) {
													$info[$field] = htmlspecialchars(array_shift($user), ENT_QUOTES);
											}
											...
											if (!isset($info['username'])) {
													$info['username'] = create_username($newstatut,
																					...
																					$prenom,
																					$_POST['prefix']);
													$info['username'] = htmlspecialchars($info['username'], ENT_QUOTES);
											}
...
function create_user($statut, $uname, $nom, $prenom, $email, $depid, $am, $phone, $lang, $send_mail)
{
        ...
        $req = db_query('SELECT * FROM user WHERE username = ' . autoquote(escapeSimple($uname)));
        ...
        $statut = htmlspecialchars($statut, ENT_QUOTES);
        ...
        $req = db_query("INSERT INTO user
                        ...
                        VALUES (" .
				autoquote(escapeSimple($nom)) . ', ' .
				autoquote(escapeSimple($prenom)) . ', ' .
				autoquote(escapeSimple($uname)) . ", '$password_encrypted', " .
				autoquote(escapeSimple($email)) .
				", $statut, $depid, " .
...
{
        $prefix = escapeSimple($prefix);
        $wildcard = escapeSimple($wildcard);
...
{
        $code = autoquote(escapeSimple($course_code));
